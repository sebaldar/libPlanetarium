...
*/
		...
		double z_sun = o_xyz.z ;
				...
				Point p = line.calculate( r * 0.999 );
			...
			if ( !cc.eof() ) {
				...
				if ( data->lookfrom.where == "earth" ) {
					...
					CelestialBody::BodyCoord coord = earth.bodyCoord ( latitude, longitude + rot );
...
*/
		...
		if ( data->lookfrom.isFromPlace ) {
			...
			if ( data->lookfrom.where == "earth" ) {
				...
				CelestialBody::BodyCoord coord = moon.bodyCoord ( latitude, longitude + rot_luna );
				...
				CelestialBody::BodyCoord coord = mars.bodyCoord ( latitude, longitude );
				...
				CelestialBody::BodyCoord coord = sun.bodyCoord ( latitude, longitude );
				...
				if ( it != data->system->bodies.end() ) {
					...
					CelestialBody & body = it->second;
					CelestialBody::BodyCoord coord = body.bodyCoord ( latitude, longitude );
...
//		double scale = data->scale;
		...
		if ( data->lookat.where == "earth" ) {
			...
			if ( !data->lookfrom.isFromPlace ) {
				...
				Point p = line.calculate( r * 0.995 );
			...
			lookat_z = 0;
				...
				Point p = line.calculate( r - r_moon  );
...
e potrebbe convergere a 0
				...
				Point p = line.calculate( r - 1000 * r_earth  );
...
e potrebbe convergere a 0
				 */
					...
					Point p = line.calculate( r - fact * 100 * r_body  );
